...
class RNN(nn.Module):
    ...
    def __init__(self, input_size):
        ...
        self.patch_size = 2
        self.rnn = nn.LSTM(self.patch_size**2, self.patch_size**2, 2, batch_first=True, bidirectional=True)
    ...
    def forward(self, x):
        ...
        out, _ = self.rnn(x)
        out = out.contiguous().view(x.size(0), self.input_size, num_patches*self.patch_size, num_patches*self.patch_size, 2)
        out = out.view(out.size(0), self.input_size, -1)
        return out  
